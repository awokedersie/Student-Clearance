    ...
    return (
        <StudentLayout user={user}>
            <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
                {/* System Status Alert */}
                <div className={`p-6 rounded-2xl border ${data?.systemActive ? 'bg-emerald-50 border-emerald-100 text-emerald-900' : 'bg-red-50 border-red-100 text-red-900'} flex items-center gap-4 shadow-sm`}>
                    <div className={`w-12 h-12 rounded-full ${data?.systemActive ? 'bg-emerald-500' : 'bg-red-500'} flex items-center justify-center text-white text-xl shrink-0 shadow-lg`}>
                        {data?.systemActive ? '✅' : '🔒'}
                    </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-6 py-4 text-gray-900 font-medium placeholder:text-gray-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all"
                                    rows={4}
                                    required
                                ></textarea>
                            </div>
                            ...
                            <button
                                type="submit"
                                disabled={submitting}
                                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-5 rounded-[24px] font-black transition-all shadow-xl shadow-indigo-600/20 uppercase tracking-widest text-sm flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-[1.01] active:scale-[0.99]"
                            >
                                {submitting ? (
                                    <>
