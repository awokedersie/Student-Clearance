                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-gray-50/50">
                                            ...
                                            <th className="px-8 py-5 text-xs font-black text-gray-400 uppercase tracking-widest">Request Date</th>
                                            ...
                                            <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                                                ...
                                                <td className="px-8 py-6">
                                                    <div className="flex justify-center">
                                                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm border ${c.status === 'approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                                ...
                                                <td className="px-8 py-6">
                                                    {c.status === 'rejected' ? (
                                                        <div className="text-xs text-red-600 font-bold bg-red-50 p-3 rounded-xl border border-red-100 max-w-xs">
                                                            {c.rejectReason}
                                                        </div>
                                                    ) : (
                                ...
                                {data?.clearances.map((c: any, i: number) => (
                                    <div key={i} className="p-6 space-y-4">
                                        <div className="flex justify-between items-start">
                                            ...
                                            <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest shadow-sm border ${c.status === 'approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                                c.status === 'rejected' ? 'bg-red-50 text-red-700 border-red-100' :
                                                    'bg-amber-50 text-amber-700 border-amber-100'
                                                }`}>
                                                {c.status}
